...
void Init_sys_timer(void)
{
  ...
  NVIC_SetPriority(INT_SysTick, 0); // Задаем высший приоритет процедуре прерывания от системного таймера
  SysTick_Config(SYS_TIMER_TICKS);  // Настраиваем частоту прверывания от системного таймера
...
void SysTick_Config(uint32_t ticks)
{
  ...
  SYSTICK->RVR  = (uint32_t)(ticks - 1UL);        // Загрузка перегружаемого значения
  SYSTICK->CVR  = 0UL;                            // Загрузка начального значения
  SYSTICK->CSR  = BIT(2) + BIT(1) + BIT(0);       // Тактирование от частоты ядра, разрешаем прерывания, включаем таймер

}

/*------------------------------------------------------------------------------
  Процедура обслуживания прерывания от системного таймера
...
/*------------------------------------------------------------------------------
  Получить текцщее время в микросекундах
 ------------------------------------------------------------------------------*/
