...
//-------------------------------------------------------------------------------------------------------
//  »нициализаци€ чипа MK66FN2M0VLQ18 на плате K66BLEZ1
//-------------------------------------------------------------------------------------------------------
int Init_MK66FN2M0VLQ18_K66BLEZ1(void)
{
  WDOG_MemMapPtr WDOG = WDOG_BASE_PTR;

  // ¬ыключать WATCHDOG надо сразу, иначе может возникнуть нестабильный запуск кода после холодного рестарта
  //--------------------------------------------------------------------------------------------------------------------------------------
  WDOG->UNLOCK = 0xC520; // ќткроем доступ на запись в регитры управлени€ WDOG
  WDOG->UNLOCK = 0xD928;
  WDOG->STCTRLH = 0
                  + LSHIFT(0x00, 14) // DISTESTWDOG | Allows the WDOGТs functional test mode to be disabled permanently| 0 WDOG functional test mode is not disabled.
                  ...
                  + LSHIFT(0x00, 0)  // WDOGEN      | Enables or disables the WDOGТs operation                         | 1 WDOG is enabled.
  ;
...
void Init_cpu(void)
{
  ...
  //--------------------------------------------------------------------------------------------------------------------------------------
  // «агрузка регистров из области чипа с заводскими установками
  ...
  MCG->C7 = 0; // OSCSEL | MCG OSC Clock Select | 0 Selects System Oscillator (OSCCLK). 1 Selects 32 kHz RTC Oscillator.
               // ѕри подключении к FLL синала 32 kHz RTC не происходит установка битов IREFS и CLKS
               // ѕоэтому подключаетс€ сигнал с OSC0 который в данной схеме принимает внешний тактовый сигнал 50 ћ√ц


  //  онфигурируем осцил€тор 0 (кварц 32 ћ√ц)
  MCG->C2 = 0
            ...
            + LSHIFT(0x02, 4) // RANGE0 | Frequency Range Select         | 1X Encoding 2 Ч Very high frequency range selected for the crystal oscillator ..
  ...
  // ”становка емкостей на осцил€торе 0
  OSC_CR = 0
  ...
  // ѕереход в режим FBE - FLL Bypassed External ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  MCG->C1 = 0
           + LSHIFT(0x02, 6) // CLKS     | Clock Source Select             | 10 Encoding 2 Ч External reference clock is selected.
  ...
  MCG->C4 = 0
           ...
           + LSHIFT(0x00, 5) // DRST_DRS | DCO Range Select                                 | 00 Encoding 0 Ч Low range (reset default).
  ...
  MCG->C6 = 0
           ...
           + LSHIFT(0x0E, 0) // VDIV    | VCO Divider                    | ”множаем на 30 (чтобы получить 180 ћ√ц от 12 ћ√ц кварца)
  ;
  ...
  while ((MCG->S & BIT(1)) == 0) // OSCINIT0 ∆дем стабилизации осцил€тора OSC0
  {
  }
  while ((MCG->S & BIT(4)) != 0) // IREFST ∆дем пока FLL начнет работать от внешнего источника тактировани€
  {
  }
  while ((MCG->S & 0x0CU) != 0x08U)  // CLKST  ∆дем пока мультиплексор CLKS установитьс€ в режим FBE - FLL Bypassed External
  {
  }

  // ѕереход в режим PBE - PLL Bypassed External ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  MCG->C6 = 0
           + LSHIFT(0x00, 7) // LOLIE0  | Loss of Lock Interrrupt Enable | 0 No interrupt request is generated on loss of lock.
           + LSHIFT(0x01, 6) // PLLS    | PLL Select                     | 1 PLLCS output clock is selected
           + LSHIFT(0x00, 5) // CME0    | Clock Monitor Enable           | 0 External clock monitor is disabled for OSC0.
           + LSHIFT(0x0E, 0) // VDIV0   | VCO0 Divider                   | ”множаем на 30 (чтобы получить 180 ћ√ц от 12 ћ√ц кварца)
  ;
  ...
  while ((MCG->S & BIT(6)) == 0x00U) // LOCK0 ∆дем пока PLL0 защелкнетс€
  {
  }

  // ѕереход в режим PEE - PLL Engaged External ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  MCG->C1 = 0
           + LSHIFT(0x00, 6) // CLKS     | Clock Source Select             | 00 Encoding 0 Ч Output of FLL or PLLCS is selected (depends on PLLS control bit).
  ...
  while ((MCG->S & 0x0CU) != 0x0CU)  // CLKST  ∆дем пока мультиплексор CLKS установитьс€ в режим PEE - PLL Engaged External
  {
  }
  ...
  //--------------------------------------------------------------------------------------------------------------------------------------
  // ”становки  Reset Control Module (RCM)
  RCM->RPFW = 0
  ...
  if ((PMC->REGSC & BIT(3)) != 0) PMC->REGSC |= BIT(3); // —брос бита ACKISO, чтобы разблокировать некоторую периферию

  //--------------------------------------------------------------------------------------------------------------------------------------
  // ”становки Power Management Controller (PMC)
  PMC->REGSC = 0
  ...
  //--------------------------------------------------------------------------------------------------------------------------------------
  // »нициализаци€ Periodic Interrupt timer
  ...
  //--------------------------------------------------------------------------------------------------------------------------------------
  // »нициализаци€ Flash Access Protection Register дл€ разрешени€ доступа к Flash по DMA и от прочих мастеров
  FMC->PFAPR = 0
  ...
  SIM->SCGC6 |= BIT(18); // ¬ключаем модуль CRC

  SIM->SCGC4 |= BIT(19); // ¬ключаем модуль CMP (аналоговые компараторы)
